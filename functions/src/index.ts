...
const showsCacheCollection = admin
  .firestore()
...
export const refreshShowsNow = onRequest(
  { ...HANDLER_REGION, concurrency: 10, invoker: "public" },
  async (req, res) => {
    ...
    const lastManual = snap.exists
      ? (snap.get("lastManualRefreshAt") as admin.firestore.Timestamp | undefined)
      : undefined;
    if (lastManual) {
...
export const searchShows = onRequest(
  { ...HANDLER_REGION, concurrency: 20, invoker: "public" },
  async (req, res) => {
    ...
    if (!query || query.trim().length < 2) {
      res
        .status(400)
        .json({ error: "query_required", message: "Query must be at least 2 characters" });
      return;
    }
...
export const saveTvdbPin = onRequest(
  { ...HANDLER_REGION, concurrency: 10, invoker: "public" },
  async (req, res) => {
    applyCors(req, res, ["POST"]);
...
export const addShow = onRequest(
  { ...HANDLER_REGION, concurrency: 20, invoker: "public" },
  async (req, res) => {
    applyCors(req, res, ["POST"]);
...
export const getEpisodes = onRequest(
  { ...HANDLER_REGION, concurrency: 20, invoker: "public" },
  async (req, res) => {
    applyCors(req, res, ["GET"]);
    ...
    try {
      ...
      const hasAllEpisodes = showCacheSnap.exists
        ? (showCacheSnap.get("hasAllEpisodes") as boolean | undefined)
        : false;

      const getCachedEpisodes = async () => {
        if (seasonNumber !== undefined) {
          const snap = await episodesRef
